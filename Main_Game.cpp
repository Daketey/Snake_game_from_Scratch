// Snakeattempt.cpp : This file contains the 'main' function. Program execution
...
int main() {
  ...
  wchar_t *screen =
      new wchar_t[nScreenWidth * nScreenHeight]; // Create Screen Buffer
  ...
  playfield =
      new unsigned char[widthoffield * heightoffield]; // Main playing field
  for (int i = 0; i < widthoffield; i++) {
    for (int j = 0; j < heightoffield; j++) {
      playfield[j * widthoffield + i] =
          (i == 0 || j == 0 || i == widthoffield - 1 || j == heightoffield - 1)
              ? 1
              : 0; // Draws a square
    }
  }
  ...
  while (!gameover) {
    ...
    for (int k = 0; k < 4; k++) {
      keys[k] = (0x8000 & GetAsyncKeyState((unsigned char)("DASW"[k]))) !=
                0; // Key input in hexdecimal form,, \x27 is right key , \x25 is
                   // left key , \x28 is down key and we have the R key. 0x8000
                   // gives real time state of the key
    }
    ...
    if (keyhold1) {
      if (speedcounter >= speed &&
          collision(currentpiece, currentrotation, currentx - 1, currenty)) {
        currentx--;
        speedcounter = 0;
      }
    }
    ...
    if (keyhold3) {
      if (speedcounter >= speed &&
          collision(currentpiece, currentrotation, currentx, currenty - 1)) {
        currenty--;
        speedcounter = 0;
      }
    }
    ...
    for (int i = 0; i < widthoffield; i++) { // Drawing the field
      for (int j = 0; j < heightoffield; j++) {
        screen[(j + 2) * nScreenWidth + (i + 2)] =
            L" #O"[playfield[j * widthoffield +
                             i]]; // Characters are written as we want them to
                                  // be displayed according to index like at
                                  // index=1 , value is "A "
      }
    }
    ...
    for (int i = 0; i < 1; i++) {
      for (int j = 0; j < 1; j++) {
        if (snake[currentpiece][i + j] == L'X') {
          ...
          if (foodpicked) {
            snakepos[positions] =
                (currenty + j) * widthoffield + (currentx + i);
    ...
    WriteConsoleOutputCharacter(
        hConsole, screen, nScreenWidth * nScreenHeight, {0, 0},
        &dwBytesWritten); // draws to console starting from postion 0,0
  }
  return 0;
}
