...
bool hitObstacle_Y(bool keyhold, int x, int y, wchar_t* screen) {
    if (keyhold) {
        if (screen[(y)*ScreenWidth + (x)] == L'O') {
            return true;
        }
    }
    return false;
...
int main() {
    ...
    int Snake_pos = current_Y * ScreenWidth + current_X;
    vector<int> snakePos;
    ...
    while (!gameOver) {
        ...
        if ((current_Y == 2 && keyHold_A == false && keyHold_D == false) || (current_Y == Height_of_Field - 3 && keyHold_A == false && keyHold_D == false)) {
            speed = 3;
        }
        else if (current_Y == 1)
        {                                                                                      // To make the collision in Y direction feel fair
            if (speedCounter > speed) {                                                        // And smoother
                ycollision = ycollision--;
            }
        }
        else if (current_Y == Height_of_Field - 2)
        {
        ...
        if (keyHold_D || keyHold_A || keyHold_S || keyHold_W) {
            ...
            if (Snake_pos == (foodPos_Y + 2) * ScreenWidth + (foodPos_X + 2))
            {
                ...
                foodPos_Y = (rand() % (Height_of_Field - 3)) + 2;
                foodPicked = true;
            }
        }
        ...
        if (hitObstacle(keyHold_D, current_X + 3, current_Y + 2, screen)) {                  //Tail and border collision in X direction
            gameOver = true;
        }
        else if (hitObstacle(keyHold_A, current_X + 1, current_Y + 2, screen)) {             // Tail and border collision in X direction
            gameOver = true;
        }
        else if (hitObstacle_Y(keyHold_S, current_X + 2, current_Y + 3, screen)) {            //Talil collison for Y direction
            gameOver = true;
        }
        else if (hitObstacle_Y(keyHold_W, current_X + 2, current_Y +1, screen)) {           //Tail Collision for Y direction
            gameOver = true;
        }
        if ((ycollision) == -3 || ycollision == 23) {          //Collison in Y direction
            gameOver = true;
        }
